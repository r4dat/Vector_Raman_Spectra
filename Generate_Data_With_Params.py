...
#     pause = input("Pause for key")
   ...
   # Raman= add_gaussian_white_noise(Raman,std)
    ...
    blur_params = np.array([sigma,SNR])
    Raman_blur, Raman = poisson_point_noise(Raman_blur, Raman,SNR)
    return Raman_blur, Raman, blur_params
...
def generate_batch(min_features,max_features,min_width,max_width,size = 10000,sigma_val=3e8,SNR=100,std_val=125e-6):
    ...
    blur_params = np.empty((size,2)) # blur parameters sigma and SNR.
    for i in range(size):
        BLUR[i,:], RAMAN[i,:], blur_params[i,:] = generate_spec(min_features, max_features, min_width, max_width,sigma_val,SNR,std_val)
    return BLUR, RAMAN, blur_params
#generate_batch(10)

def generate_all_data(min_features,max_features,min_width,max_width,N_train,N_valid):
    BLUR_train, RAMAN_train, blur_train_params = generate_batch(min_features,max_features,min_width,max_width,N_train) # generate bactch for training
    BLUR_valid, RAMAN_valid, blur_valid_params = generate_batch(min_features,max_features,min_width,max_width,N_valid) # generate bactch for validation
    return BLUR_train, RAMAN_train, BLUR_valid, RAMAN_valid, blur_train_params, blur_valid_params
...
def generate_and_save_data(N_train,N_valid,fname='./data/',a=1,b='a',sigma_val=3e8,std_val=125e-6):
    ...
    BLUR_train, RAMAN_train, BLUR_valid, RAMAN_valid, blur_train_params, blur_valid_params = generate_all_data(min_features,max_features,min_width,max_width,N_train,N_valid)
    ...
    print(np.isnan(blur_train_params).any())
    print(np.isnan(blur_valid_params).any())

    sigma_val="known_blur_"
    std_val="_"
    ...
    pd.DataFrame(blur_valid_params).to_csv(
        fname + str(a) + b + 'sigma' + str(sigma_val) + 'std' + str(std_val) + 'blur_valid_params.csv')
    pd.DataFrame(blur_train_params).to_csv(
        fname + str(a) + b + 'sigma' + str(sigma_val) + 'std' + str(std_val) + 'blur_train_params.csv')
...
if __name__=='__main__':
    generate_and_save_data(N_train=200,N_valid=200,fname='./data/',a=4,b='d',sigma_val=2e8,std_val=125e-4) #10
